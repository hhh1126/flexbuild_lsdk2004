...
define repo-update
	@for tree in $2; do \
		...
		elif [ -n "$$tag" ]; then git clone --recurse-submodules $$repourl $$tree && cd $$tree && git checkout $$tag -b $$tag && cd -; \
		elif [ -n "$$commit" ]; then git clone --recurse-submodules $$repourl $$tree && cd $$tree && git checkout $$commit -b $$commit && cd -; \
