...
# LS1088ARDB_PB, LS2088ARDB, LX2160ARDB, LX2160ARDB_REV2
#
...
if test -z "$linux_itb_img"; then
    if test $board = ls1021atwr; then
	linux_itb_img=/lsdk2004_yocto_tiny_LS_arm32.itb
    else
	linux_itb_img=/lsdk2004_yocto_tiny_LS_arm64.itb
    fi
fi
...
elif test $board = ls1012ardb -o $board = ls1028ardb -o $board = ls1046ardb -o $board = ls1046afrwy -o $board = ls1088ardb_pb -o $board = lx2160ardb -o $board = lx2160ardb_rev2; then
    # SPI flash on LS1012ARDB, LS1046ARDB,  LS1046AFRWY, LS1088ARDB_PB, LX2160ARDB
    addr_bl2=0x0
    addr_fip=0x00100000
    addr_ddr=0x00800000
...
if test $board = ls1046ardb -o $board = ls1046afrwy -o $board = ls1088ardb_pb -o $board = lx2160ardb -o $board = lx2160ardb_rev2; then
    if test $bank = other; then
...
# DPAA2 MC firmware
if test $board = ls1088ardb_pb -o $board = ls2088ardb -o $board = lx2160ardb -o $board = lx2160ardb_rev2; then
    if load $bd_type $bd_part $img_load_addr $mcfw_img && $pt_off_cmd $addr_mcfw $efs && $erase_cmd $addr_mcfw +$filesize && $write_cmd $img_load_addr $addr_mcfw $filesize; then
	echo Success: flashed $mcfw_img to $addr_mcfw
    else
	echo Failed to flash $mcfw_img
